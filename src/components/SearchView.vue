                ...
                <template #default>
                  <div v-for="(searchItem, index) in dataList" :key="index">
                    <el-card shadow="hover" style="margin-top: 10px">
                      <div style="font-size: 20px" @click="
                      showMoreWordInfo(index);
                      searchWordFun(searchItem.wordId as string);
                      currentWord = searchItem">
                        <el-text :style="`font-size: var(--el-font-size-extra-large)`">{{ searchItem.word }} |
                          {{ searchItem.hiragana }} {{ searchItem.pronunciation }}|{{searchItem.dictionaryNames}}
                        </el-text>
                      </div>
                    </el-card>
                  </div>
...
const {searchWord, dataList,wordResult,currentWord} = storeToRefs(wordStore)
